...
/**
 * Class: mxIBM2MondrianLegendItem
 * Extends <mxShape> to implement shapes that provide a legend that is compliant with the IBM Mondrian Design Method
 */
 function mxIBM2MondrianLegendItem(bounds, fill, stroke, strokewidth)
 {
	 mxShape.call(this);
	 this.bounds = bounds;
	 this.fill = fill;
 ...
 mxUtils.extend(mxIBM2MondrianLegendItem, mxShape);
 
 mxIBM2MondrianLegendItem.legendItemHeight = 16;

 mxIBM2MondrianLegendItem.prototype.cst = 
 {
		MONDRIAN_LEGEND_ITEM : 'mxgraph.ibm2mondrian.legendItem',
		LEGEND_COLOR : 'legendColor',
		LEGEND_COLOR_DEFAULT : 'gray:white:white',
		LEGEND_SHAPE_TYPE: 'shapeType',
		LEGEND_SHAPE_TYPE_DEFAULT: 'pn'
 };
 
 mxIBM2MondrianLegendItem.prototype.customProperties = [
	 {name: 'legendColor', dispName: 'Color', type: 'enum', defVal: 'gray:white:white',
		enumList:[
			{val:'gray:white:white', dispName: 'Text: Gray'},
			{val:'black:white:white', dispName: 'Text: Black'},
			{val:'gray:gray:white', dispName: 'Text & Line: Gray'},
 ...
 mxIBM2MondrianLegendItem.prototype.init = function(container)
 {
	 let mondrianAttributes = ['Legend-Item-Title'];
	 for (attributeIndex = 0; attributeIndex < mondrianAttributes.length; attributeIndex++ ) {
		 if(!this.state.cell.hasAttribute(mondrianAttributes[attributeIndex]))
		 {
			 this.state.cell.setAttribute(mondrianAttributes[attributeIndex],'')
 ...
 mxIBM2MondrianLegendItem.prototype.paintVertexShape = function(c, x, y, w, h)
 {	
	 let legendColor = mxUtils.getValue(this.style, 
		mxIBM2MondrianLegendItem.prototype.cst.LEGEND_COLOR, mxIBM2MondrianLegendItem.prototype.cst.LEGEND_COLOR_DEFAULT).split(':');
 
	 let shapeType = mxUtils.getValue(this.style, 
			mxIBM2MondrianLegendItem.prototype.cst.LEGEND_SHAPE_TYPE, mxIBM2MondrianLegendItem.prototype.cst.LEGEND_SHAPE_TYPE_DEFAULT);
	 ...
	 this.paintLegendItemShape(c, shapeType);
 ...
 mxIBM2MondrianLegendItem.prototype.paintLegendItemShape = function(c,shapeType)
 {
	const cornerRadius = 4;
	const targetSystemRadius = 8;
	let shapeWidth = 16;
	const shapeHeigth = 16;

	let standardStrokeColor = mxIBM2MondrianBase.prototype.getSelectedColorSpecification('gray').swatch_50;
	c.setStrokeColor(standardStrokeColor);
	...
	else if(shapeType == 'ts')
	{
		shapeWidth = 20;
		c.begin();
		c.moveTo(targetSystemRadius, 0);
		c.lineTo(shapeWidth - targetSystemRadius, 0);
		c.arcTo(targetSystemRadius, targetSystemRadius, 0, 0, 1, shapeWidth - targetSystemRadius, shapeHeigth);
		c.lineTo(targetSystemRadius, shapeHeigth);
		c.arcTo(targetSystemRadius, targetSystemRadius, 0, 0, 1, targetSystemRadius, 0);
		c.close();
	}
	else if(shapeType == 'ln' || shapeType == 'lc')
	{
		c.roundrect(0,0,16,16,cornerRadius,cornerRadius);
	}
	else if(shapeType === 'lg')
	{
		c.begin();
		c.moveTo(0, 0);
		c.lineTo(shapeWidth - cornerRadius, 0);
		c.arcTo(cornerRadius, cornerRadius, 0, 0, 1, shapeWidth, cornerRadius);
		c.lineTo(shapeWidth, shapeHeigth - cornerRadius);
		c.arcTo(cornerRadius, cornerRadius, 0, 0, 1, shapeWidth - cornerRadius, shapeHeigth);
		c.lineTo(cornerRadius, shapeHeigth);
		c.arcTo(cornerRadius, cornerRadius, 0, 0, 1, 0, shapeHeigth - cornerRadius);
		c.lineTo(0, 0);
		c.close();
	}
	else
...
// Component decorator
if(shapeType === 'lc' || shapeType === 'pc')
{
	let componentDecoratorOffset = -2;
	let componentDecoratorHeight = 2;
	let componentDecoratorWidth = 4;

	c.save();
	c.setDashed(false);
	c.setFillColor('#ffffff');
	c.rect(componentDecoratorOffset, 4, componentDecoratorWidth, componentDecoratorHeight);
	c.fillAndStroke();
	c.rect(componentDecoratorOffset, 10, componentDecoratorWidth, componentDecoratorHeight);
	c.fillAndStroke();
	c.restore();
}
...
//group bar
	if(shapeType == 'pg' || shapeType == 'lg')
	{
		c.setFillColor(standardStrokeColor);
		c.rect(0, 0, 2, 12);
		c.fillAndStroke();
	}
 }

 mxIBM2MondrianLegendItem.prototype.getLabelBounds = function(rect)
 {
	 const legendTitleHeight = 16;
	 const shapeSize = 16;
	 const spacer = 8; 
	 return new mxRectangle(
					 rect.x + (shapeSize + spacer) * this.scale, 
					 rect.y,
					 rect.width - (shapeSize + spacer) * this.scale,
					 legendTitleHeight * this.scale);
 };
...
mxCellRenderer.registerShape(mxIBM2MondrianLegendItem.prototype.cst.MONDRIAN_LEGEND_ITEM, mxIBM2MondrianLegendItem);